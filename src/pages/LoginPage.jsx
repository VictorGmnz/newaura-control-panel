...
function pickFirstRoute(allowedPages) {
  const order = [
    "dashboard",
    "messages",
    "feedback",
  ...
  for (const key of order) {
    if (allowedPages.includes(key) && ROUTE_BY_KEY[key]) return ROUTE_BY_KEY[key];
  }
  return "/";
}
...
export default function LoginPage() {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen" style={{ background: "#586C83" }}>
      <div
        className="relative bg-white shadow-2xl rounded-3xl p-8 w-full max-w-sm flex flex-col items-center"
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col items-center mt-4">
          <h2 className="text-2xl font-bold mb-6 text-center text-primary mt-8">Login</h2>
          {error &&
            error.split("\n").map((line, idx) => (
